...
from replay_parser import parse_replay_header, parse_replay_header_raw, find_user_result
...
def debug_show_names(root, cfg):
    ...
    if not isimler:
        metin += "  (Hiç oyuncu bulunamadı)\n\n"
        try:
            raw = parse_replay_header_raw(en_yeni)
            metin += f"Header'da bulunan HAM alan adları ({len(raw)} adet):\n"
            for k in raw.keys():
                metin += f"  - {k}\n"
        except Exception as e:
            metin += f"Ham alanlar da okunamadı: {e}\n"
    elif all(isim == "?" for isim in isimler):
        metin += "\n(Oyuncular bulundu ama isim alanı boş geldi — ilk oyuncunun HAM verisi:)\n"
        try:
            raw = parse_replay_header_raw(en_yeni)
            raw_players = raw.get("PlayerStats", [])
            if raw_players and isinstance(raw_players[0], dict):
                for k, v in raw_players[0].items():
                    metin += f"  {k!r}: {v!r}\n"
            else:
                metin += "  (PlayerStats ham veride de boş/liste değil)\n"
        except Exception as e:
            metin += f"Ham veri okunamadı: {e}\n"
